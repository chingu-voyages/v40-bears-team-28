...
export class UserBooksModel {
  ...
  async showByUserId(id: string): Promise<UserBook[]> {
    try {
      const connect = await database.connect()
      const sql = `SELECT b.book_id AS id, b.authors, b.image, b.title, b.description, b.url
                   FROM user_books
  ...
  async showByUserIdAndBookId(userId: string, bookId: string): Promise<UserBook> {
    try {
      const connect = await database.connect()
      const sql = `SELECT *
                   FROM user_books
  ...
  async delete(userId: string, bookId: string): Promise<UserBook> {
    try {
      const connect = await database.connect()
      const sql = `DELETE
                   FROM user_books
